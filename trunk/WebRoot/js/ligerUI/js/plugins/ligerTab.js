...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        overrideTabItem: function (targettabid, options)
        {
            ...
            alert(tabid);
           // 如果传入option没有设置tabid，则 设置为 targettabid. 去掉if (tabid == undefined) tabid = g.getNewTabid();这样子 也可以自定义新tab的tabid，也可以不定义
            //if(tabid == undefined) tabid = targettabid;
        ...
        reload: function (tabid)
        {
            ...
            var url = $(iframe).attr("src");
             ...
             //alert(id);
           // alert($(document.getElementById(id).contentWindow.document.body).find("searchButton").id);
            //$(document.getElementById(id).contentWindow.document.getElementById("btnOk"))\
