	 ...
	 <script type="text/javascript">
		...
		var openHeight="650";
 		...
 		function loadFl(xcode,ycode){
 			...
 			//window.open('<%=basePath%>UI/enterinfo/flfj_listInfo.action?distcode='+xcode+'&ZYBWHLB=0&hylb='+hylb+'&whys='+whys+"&nianfen="+year+"&distj=3");
 			try {
			  myWindow = top.$.ligerDialog.open({url:webpath+"/UI/zyjk/flfj/tjflfj.jsp?distcode="+xcode+"&lx=0&hyfl="+hylb+"&whys="+whys+"&nianfen="+year+"&zbid=", width: openWidth,height:openHeight, title:"自查自纠列表",
 		...
 		function loadhg(xcode,ycode){
 			...
 			try {
			  myWindow = top.$.ligerDialog.open({url:webpath+"/UI/zyjk/flfj/tjflfj.jsp?distcode="+xcode+"&lx=1&hyfl="+hylb+"&whys="+whys+"&nianfen="+year+"&zbid=", width: openWidth,height:openHeight, title:"自查自纠列表",
			 showMax: true, showToggle: true, showMin: false, isResize: true, slide: false }); 
			  }catch (e) {
				$.ligerDialog.error(e.message);
			  }
 		...
 		function loadbhg(xcode,ycode){
 			...
 			try {
			  myWindow = top.$.ligerDialog.open({url:webpath+"/UI/zyjk/flfj/tjflfj.jsp?distcode="+xcode+"&lx=2&hyfl="+hylb+"&whys="+whys+"&nianfen="+year+"&zbid=", width: openWidth,height:openHeight, title:"自查自纠列表",
			 showMax: true, showToggle: true, showMin: false, isResize: true, slide: false }); 
			  }catch (e) {
				$.ligerDialog.error(e.message);
			  }
 		...
 		function openZjTj(distcode){
 			...
 			window.open('<%=basePath%>UI/action/flfjtj/FLFJTjAction_getZjDistcodeTable.action?distcode='+distcode+'&hylb='+hylb+'&whys='+whys+"&nianfen="+year+"&distj=3");
  	...
  	<br>
    	...
    	<table  class="analyzetbl" width="100%" align="center" border="0">
    		...
    		%>
    			<tr>
    				<td align="center">
    					<%if(dt.getRowMeta(i).contains("00")||dt.getRowMeta(i).equals("")){ %>
