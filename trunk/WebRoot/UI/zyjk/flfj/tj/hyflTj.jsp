...
<html>
	  ...
	  var myWindow = null;
		...
		var openHeight="650";
 		...
 		function loadFl(xcode,ycode){
 			...
 			//window.open('<%=basePath%>UI/zyjk/flfj/tjflfj.jsp?distcode='+distcode+'&ZYBWHLB='+ycode+'&hylb='+xcode+'&whys='+whys+"&nianfen="+year);
 			try {
			  myWindow = top.$.ligerDialog.open({url:webpath+'/UI/zyjk/flfj/tjflfj.jsp?distcode='+distcode+'&ZYBWHLB='+ycode+'&hylb='+xcode+'&whys='+whys+'&nianfen='+year, width: openWidth,height:openHeight, title:"自查自纠列表",
 		...
 		function loadWFl(xcode,ycode){
 			...
 			var whys=document.getElementById("whys").value;
 			var year=document.getElementById("year").value;
 			//window.open('<%=basePath%>UI/zyjk/flfj/tjflfj.jsp?distcode='+distcode+'&ZYWSGLDJ='+ycode+'&hylb='+xcode+'&whys='+whys+"&nianfen="+year);
 			try {
			  myWindow = top.$.ligerDialog.open({url:webpath+'/UI/zyjk/flfj/tjflfj.jsp?distcode='+distcode+'&ZYWSGLDJ='+ycode+'&hylb='+xcode+'&whys='+whys+'&nianfen='+year, width: openWidth,height:openHeight, title:"自查自纠列表",
			 showMax: true, showToggle: true, showMin: false, isResize: true, slide: false }); 
			  }catch (e) {
				$.ligerDialog.error(e.message);
			  }
 		...
 		function loadFj(xcode,ycode){
 			...
 			var whys=document.getElementById("whys").value;
 			var year=document.getElementById("year").value;
 			//window.open('<%=basePath%>UI/zyjk/flfj/tjflfj.jsp?distcode='+distcode+'&HDGLDJ='+ycode+'&hylb='+xcode+'&whys='+whys+"&nianfen="+year);
 			try {
			  myWindow = top.$.ligerDialog.open({url:webpath+'/UI/zyjk/flfj/tjflfj.jsp?distcode='+distcode+'&HDGLDJ='+ycode+'&hylb='+xcode+'&whys='+whys+'&nianfen='+year, width: openWidth,height:openHeight, title:"自查自纠列表",
			 showMax: true, showToggle: true, showMin: false, isResize: true, slide: false }); 
			  }catch (e) {
				$.ligerDialog.error(e.message);
			  }
  ...
  <body>
   <form action="<%=basePath %>/UI/action/flfjtj/FLFJTjAction_gethylbTable.action" id="tform" name="tform">
  ...
  <br>
  	<center><font size="5">按行业分类对企业分类分级统计</font></center>
  	<br>
    	<table  class="analyzetbl" width="100%" align="center" border="0">
    		<tr>
    			
    			<th rowspan="2" width="10%">行业分类</th>
