 ...
 ***********************************************************************/
	 ...
	 $(function (){
	  ...
	  /*TREE*/
	   $("#tree").ligerTree({         
                //data:[{id:'1',pid:'0',children:[],text:'企业基本情况',isexpand:'false'}],
                idFieldName :'id',
                parentIDFieldName :'pid', 
                nodeWidth : 200,
                checkbox: false,
                slide: false,   
                url:webpath+"/st/system/action/tree/tree_getTree.action?strTreeName=EnterinfoTree",                     
                onExpand:onExpand,
      ...
      function onBeforeExpand(node){
            ...
            if(node.data.children && (node.data.children.length == 0)){
               tree.loadData(node.target,webpath+"/st/system/action/tree/tree_getTree.action?strTreeName=EnterinfoTree");
            }  
       }         
     /*查看*/
        function t_enter(node){
        try{ 
           //alert("corpkey:"+$("#corpkey").val());
          	f_addTab(node.data.id,node.data.text,webpath+node.data.url+"?messageBean.method=look&enterno="+$("#corpkey").val()+"&strSysno="+$("#corpkey").val());
        }catch (e) {
     		   $.ligerDialog.error(e.message);
    		}
