     ...
     /*查看*/
        ...
        try{
          	...
          	if(node.data.id=='2'||node.data.id=='5'){
          	   f_addTab(node.data.id,node.data.text,webpath+node.data.url+"?messageBean.method=add&enterno="+ getUrlParam("enterno") +"&strSysno="+getUrlParam("sysno")+"&bgbh="+getUrlParam("sysno"));        	 
          	}else{
