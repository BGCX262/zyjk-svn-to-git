...
try{
 ...
 { text: '查看机构信息', click:f_lookjg,  icon:'pager' }    
  ...
  var enterno=getUrlParam("enterno"); 
  var xmbh=getUrlParam("strSysno");
   if (xmbh==null||xmbh==""){
   			grid =  Grid('pjbgjbqkGrid','检测机构评价报告管理',toolbar);  
 		
 	}else{
 	
 		//grid = sub_Grid('pjbgjbqkGrid','检测、评价报告与记录',toolbar," and jsdwbh like '%"+ getUrlParam("strSysno") +"%' and xmbh like '%"+getUrlParam("sysno")+"%'");
 		grid = sub_Grid('pjbgjbqkGrid','检测、评价报告与记录',toolbar," and  xmbh like '%"+getUrlParam("strSysno")+"%'");
 		
 		//document.getElementById("jgno").value=jsdwbh;  
 		document.getElementById("xmbh").value=xmbh;  
 		document.getElementById("enterno").value=enterno;
 	} 
...
$.ligerDialog.error(e.message);
     ...
     function f_search(){
            ...
            $.ligerDialog.confirm(MSG_DELETE_CONFRIM, function (state)
              {
               ...
               {    
                    ...
                    $.post(webpath+"/UI/action/zyjk/JsxmjcbgAction_delete.action",{strSysno:strId},function(data){
        ...
        myWindow = top.$.ligerDialog.open({ url: webpath+"/UI/action/zyjk/JsxmjcbgAction_findByKey.action?messageBean.method=add&xmbh="+getUrlParam("strSysno")+"&strSysno="+strId+"&jsdwbh="+getUrlParam("enterno"),  width: openWidth,height:openHeight, title: '检测、评价报告与记录',
        ...
        if (strId==null||strId.length == 0) {
         ...
         myWindow = top.$.ligerDialog.open({url: webpath+"/UI/action/zyjk/JsxmjcbgAction_findByKey.action?messageBean.method=update&strSysno="+strId, width: openWidth,height:openHeight, title: '检测、评价报告与记录',
         showMax: false, showToggle: false, showMin: false, isResize: false, slide: false }); 
         //f_search(); 
        }
        }catch (e) {
        ...
        function f_look()
        {
          try {
             ...
             if (selected==null||selected.length == 0) {
                 ...
                 //showMax: true, showToggle: true, showMin: false, isResize: true, slide: false }); 
              //top.tab.addTabItem ({tabid : top.tab.getNewTabid(),text: selected.jsdwmc+'报告信息', url:webpath+'/UI/zyjk/jcjg/jcjgbg/jcjgbgmain.jsp?sysno='+ selected.sysno +'&enterno='+selected.enterno });        
             loadSubWindow(webpath+'/UI/zyjk/jcjg/jcjgbg/jcjgbgmain.jsp?sysno='+ selected.sysno +'&enterno='+selected.enterno);
             }
          }catch (e) {
                 $.ligerDialog.error(e.message);
          }
