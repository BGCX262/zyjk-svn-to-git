...
try{
 ...
 try {
  myWindow = top.$.ligerDialog.open({url:webpath+"/UI/action/jcqk/JcqkAction_findByKey.action?messageBean.method=look&enterno="+ getUrlParam("enterno") +"&strSysno="+data.sysno, width: openWidth,height:openHeight, title:"劳动者职业史",
...
$.ligerDialog.error(e.message);
     ...
     function f_search(){
        ...
        if (strId==null||strId.length == 0) {
         ...
         myWindow = top.$.ligerDialog.open({url: webpath+"/UI/action/zyjk/JcqkAction_findByKey.action?messageBean.method=update&enterno="+ getUrlParam("enterno") +"&strSysno="+strId, width: openWidth,height:openHeight, title: '劳动者职业史',
         showMax: false, showToggle: false, showMin: false, isResize: false, slide: false }); 
        // f_search(); 
        }
        }catch (e) {
        ...
        function f_look()
        {
          try {
           ...
           if (selected==null||selected.length == 0) {
                 ...
                 myWindow = top.$.ligerDialog.open({url: webpath+"/UI/action/zyjk/JcqkAction_findByKey.action?messageBean.method=look&enterno="+ getUrlParam("enterno") +"&strSysno="+selected.sysno, width: openWidth,height:openHeight, title: '劳动者职业史',
                 showMax: false, showToggle: false, showMin: false, isResize: true, slide: false }); 
             }
       }catch (e) {
                 $.ligerDialog.error(e.message);
