        ...
        try {
           ...
           $.post(webpath+"/st/system/action/table/table_getColumn.action",{key:'cxtjjgGrid'},function(data){
			 ...
			 pageSize:15,
			  onSelectRow:function (data, rowindex, rowobj)
                {      
                       $("#zldwbh").val(data.sysno);
                       $("#zldwmc").val(data.jgmc);
                      // $("#dwdz").val(data.zcdz);
                       //$("#sex").val(data.sex);
          ...
          var columntree  =colselect.grid.ligerGetGridManager();       
             */
             ...
             if(method=="add"){
               actionurl=webpath+"/UI/action/ldzkfqk/LdzkfqkAction_insert.action";
             }else if(method=="look"){
               $("#subButton").hide();
               $("#colButton").hide();
             }else if(method=="update"){
                actionurl=webpath+"/UI/action/ldzkfqk/LdzkfqkAction_update.action";
             } 
