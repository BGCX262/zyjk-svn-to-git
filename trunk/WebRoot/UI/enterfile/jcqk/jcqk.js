...
try{
 ...
 grid = sub_Grid('jcqkGrid','检测情况',toolbar," and dwbh like '%"+ getUrlParam("dwbh") +"%' and bgbh like '%"+ getUrlParam("bgbh") +"%'");
        }catch (e) {
...
$.ligerDialog.error(e.message);
     ...
     function f_search(){
         try{
              var strWhere = " and dwbh like '%"+ getUrlParam("dwbh") +"%' and bgbh like '%"+ getUrlParam("bgbh") +"%'";           
        ...
        //alert("bgbh2:"+getUrlParam("bgbh"));
        myWindow = top.$.ligerDialog.open({ url: webpath+"/UI/action/jcqk/JcqkAction_findByKey.action?messageBean.method=add&bgbh="+ getUrlParam("bgbh") +"&dwbh="+ getUrlParam("dwbh") +"&ldzbh="+getUrlParam("strSysno")+"&strSysno="+strId,  width: openWidth,height:openHeight, title: '检测情况',
        showMax: false, showToggle: false, showMin: false, isResize: false, slide: false }); 
          // f_search();   
        }catch (e) {
                $.ligerDialog.error(e.message);
        ...
        if (strId==null||strId.length == 0) {
         ...
         myWindow = top.$.ligerDialog.open({url: webpath+"/UI/action/jcqk/JcqkAction_findByKey.action?messageBean.method=update&enterno="+ getUrlParam("enterno") +"&strSysno="+strId, width: openWidth,height:openHeight, title: '检测情况',
         showMax: false, showToggle: false, showMin: false, isResize: false, slide: false }); 
        // f_search(); 
        }
        }catch (e) {
        ...
        function f_look()
        {
          try {
           ...
           if (selected==null||selected.length == 0) {
                 ...
                 myWindow = top.$.ligerDialog.open({url: webpath+"/UI/action/jcqk/JcqkAction_findByKey.action?messageBean.method=look&enterno="+ getUrlParam("enterno") +"&strSysno="+selected.sysno, width: openWidth,height:openHeight, title: '检测情况',
                 showMax: false, showToggle: false, showMin: false, isResize: false, slide: false }); 
             }
       }catch (e) {
                 $.ligerDialog.error(e.message);
