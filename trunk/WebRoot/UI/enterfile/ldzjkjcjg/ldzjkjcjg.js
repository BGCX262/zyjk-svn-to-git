...
try{
 ...
 grid = sub_Grid('qyzywsjkjcjgGrid','劳动者健康检查结果',toolbar," and ldzbh like '%" + getUrlParam("strSysno") + "%' ");
        }catch (e) {
...
$.ligerDialog.error(e.message);
     ...
     function f_search(){
        ...
        myWindow = top.$.ligerDialog.open({ url: webpath+"//UI/action/ldzjkjcjg/LdzjkjcjgAction_findByKey.action?messageBean.method=add&enterno="+ getUrlParam("enterno") +"&ldzbh="+getUrlParam("strSysno")+"&strSysno="+strId,  width: openWidth,height:openHeight, title: '劳动者健康检查结果',
