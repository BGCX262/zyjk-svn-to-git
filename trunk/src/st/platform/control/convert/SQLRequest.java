                    ...
                    for(int j = 0; j < recorderChildList.size(); j++)
                    {
                         ...
                         for(int k = 0; k < fieldChildList.size(); k++)
                         {
                              ...
                              if(fieldChildNode.getAttribute("oldValue") != null)
                                   fieldRequest.setoldValue(DBUtil.toDB(Basic.decode(fieldChildNode.getAttributeValue("oldValue")).trim()));
                              else
                                   fieldRequest.setoldValue("");

                              if(fieldChildNode.getAttribute("fieldLable") != null){

                                   fieldRequest.setfieldLable(DBUtil.toDB(Basic.decode(fieldChildNode.getAttributeValue("fieldLable"))));
                              }
                             else
                              ...
                              fieldRequest.setValue(DBUtil.toDB(Basic.decode(fieldChildNode.getAttributeValue("value")).trim()));
                              //填充数据
